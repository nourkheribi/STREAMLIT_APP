...
def charger_modele_depuis_zip():
    zip_path = "rf2.pkl.zip"  # Nom du fichier ZIP mis à jour
    model_path = "models/rf2.pkl"  # Le modèle se trouve dans 'models/rf2.pkl'
    os.makedirs("models", exist_ok=True)

    # Vérifier si le modèle est déjà extrait
    if not os.path.exists(model_path):
        if not os.path.exists(zip_path):
            st.sidebar.error("❌ Le fichier 'rf2.pkl.zip' est manquant.")
            st.stop()
